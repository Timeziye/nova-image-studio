...
**开源版特性：**
- 支持 **4 套提供商配置**：绘图 Google / 绘图 OpenAI / 文本 Google / 文本 OpenAI
...
- **提供商**：支持分别配置绘图 Google、绘图 OpenAI、文本 Google、文本 OpenAI 的 API Key 和 Base URL
...
> 首次启动时需要在 UI 的"设置"中分别配置绘图 / 文本提供商的 API Key 和 Base URL，并设置图片模型与默认模型。所有前端配置均保存在浏览器 localStorage，可通过备份功能导出。
