...
- **Image 2 额外参数**：透明背景、质量、风格参数可按模型开启或关闭，并支持默认开启
...
> 首次启动时需要在 UI 的"设置"中配置 API 提供商（Google / OpenAI 的 API Key 和 Base URL）、图片模型和默认模型。所有前端配置均保存在浏览器 localStorage，可通过备份功能导出。
